...
export default function Sidebar() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [indexRes, summaryRes, articlesRes] = await Promise.all([
          fetch("/api/index"),
          fetch("/api/summary"),
  ...
  return (
    <>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="relative pt-4">
              <input
                ...
                className="w-full rounded-full bg-secondary px-4 py-3 pr-12 text-sm focus:outline-none focus:ring-2 focus:ring-ring cursor-pointer"
                onClick={() => setIsChatOpen(true)}
                readOnly
              />
              <button
                className="absolute right-2 top-1/2 -translate-y-1/2 translate-y-2 flex h-9 w-9 items-center justify-center rounded-full bg-foreground text-background"
                onClick={() => setIsChatOpen(true)}
              >
      ...
      <AIChatModal isOpen={isChatOpen} onClose={() => setIsChatOpen(false)} newsContext={newsContext} />
    </>
  )
}
